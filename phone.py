...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        btn_open_dialog = tk.Button(toolbar, text='Добавить контакт', command=self.open_dialog, bg='white',
                                    bd=0, compound=tk.TOP, image=self.add_img)
        btn_open_dialog.pack(side=tk.LEFT)

        self.update_img = tk.PhotoImage(file='edit.png')
        btn_edit_dialog = tk.Button(toolbar, text='Редактировать', bg='white', bd=0, image=self.update_img,
                                    compound=tk.TOP, command=self.open_update_dialog)
        btn_edit_dialog.pack(side=tk.LEFT)

        self.delete_img = tk.PhotoImage(file='delete.png')
        btn_delete_dialog = tk.Button(toolbar, text='Удалить', bg='white', bd=0, image=self.delete_img,
                                      compound=tk.TOP, command=self.delete_records)
        btn_delete_dialog.pack(side=tk.LEFT)

        self.search_img = tk.PhotoImage(file='search.png')
        btn_search = tk.Button(toolbar, text='Поиск', bg='white', bd=0, image=self.search_img,
        ...
        btn_refresh = tk.Button(toolbar, text='Обновить', bg='white', bd=0, image=self.refresh_img,
        ...
        self.tree.heading('fio', text='Имя контакта')
        self.tree.heading('category', text='Тип телефона')
